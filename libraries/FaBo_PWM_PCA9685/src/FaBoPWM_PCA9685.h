...
public:
	...
	void init(uint8_t addr, uint8_t value = PCA9685_DEFAULT_VALUE);
	uint16_t calc_prescale(uint16_t hz);
	void set_hz(uint8_t addr, uint16_t hz);
	void calc_hz(uint16_t prescale, uint16_t *value);
	void set_channel_value(uint8_t addr, uint8_t channel, uint16_t value);
	uint16_t get_channel_value(uint8_t addr, uint8_t channel);
private:
	boolean checkI2c(uint8_t addr, uint8_t register_addr);
	void writeI2c(uint8_t addr, uint8_t register_addr, uint8_t value);
	void readI2c(uint8_t addr, uint8_t register_addr, int num, uint8_t *buf);
};
